...
namespace LeetCodePractice.CommonTools
{
    public class RedisHelper
    {
        ...
        public RedisHelper(string connectionString, string defaultKey, int db = 0)
        {
        ...
        {
            LogHelper.Error($"{nameof(ConnMultiplexer_ConfigurationChangedBroadcast)}: {e.EndPoint}");
        }
        ...
        {
            LogHelper.Error($"{nameof(ConnMultiplexer_InternalError)}: {e.Exception}");
        }
        ...
        {
            LogHelper.Error($"{nameof(ConnMultiplexer_ConfigurationChanged)}: {e.EndPoint}");
        }
        ...
        {
            LogHelper.Error($"{nameof(ConnMultiplexer_ConnectionFailed)}: {e.Exception}");
        }
        ...
        {
            LogHelper.Error($"{nameof(ConnMultiplexer_ConnectionRestored)}: {e.Exception}");
        }
        ...
        /// <param name="list">需要进行插入的数据list</param>
        /// <param name="expiry">过期时间</param>
        /// <returns></returns>
        public async Task<bool> SetBatchData<T>(string key, List<T> list, TimeSpan? expiry = default(TimeSpan?))
        {
            try
            {
                ...
                {
                    await batch.SortedSetAddAsync(key, JsonConvert.SerializeObject(list[i]), i);
                }
                batch.Execute();
                return true;
            }
        ...
        /// <typeparam name="T"></typeparam>
        /// <param name="key">key值</param>
        /// <param name="strat">起始行</param>
        /// <param name="stop">终止行</param>
        /// <returns></returns>
        public List<T> GetBatchData<T>(string key, int strat = 0, int stop = -1)
        {
            RedisValue[] value = _db.SortedSetRangeByRank(key, strat, stop);
            List<T> list = JsonConvert.DeserializeObject<List<T>>(value.ToString());
            return list;
        }
        #endregion
        #endregion
        #region MyRegion
        public RedisValue[] TestHash()
        {
            //Random s = new Random();
            //var batch = _db.CreateBatch();
            //for (int i = 0; i < 1000000; i++)
            //{
            //    batch.HashSetAsync("user:id:" + i, "id", i);
            //    batch.HashSetAsync("user:id:" + i, "name", "z" + i);
            //    batch.HashSetAsync("user:id:" + i, "age", s.Next(1, 100));
            //    batch.ListLeftPushAsync("ids:user", i);
            //}
            //batch.Execute();
            //_db.HashSet("user:id:1", "id", "1");
            //_db.HashSet("user:id:1", "name", "z1");
            //_db.HashSet("user:id:1", "age", 18);
            //_db.HashSet("user:id:2", "id", "2");
            //_db.HashSet("user:id:2", "name", "z2");
            //_db.HashSet("user:id:2", "age", 16);
            //_db.HashSet("user:id:3", "id", "3");
            //_db.HashSet("user:id:3", "name", "z3");
            //_db.HashSet("user:id:3", "age", 20);
            //_db.HashSet("user:id:4", "id", "4");
            //_db.HashSet("user:id:4", "name", "z4");
            //_db.HashSet("user:id:4", "age", 16);
            //_db.ListLeftPush("ids:user", 1);
            //_db.ListLeftPush("ids:user", 2);
            //_db.ListLeftPush("ids:user", 3);
            //_db.ListLeftPush("ids:user", 4);
            RedisValue[] abc = new RedisValue[3] { "user:id:*->id", "user:id:*->age", "user:id:*->name" };
            RedisValue[] Result = _db.Sort("ids:user", 150000, 20, Order.Ascending, SortType.Numeric, "user:id:*->age", abc);
            return Result;
        }
        #endregion
        ...
        //        _db = _connections.GetOrAdd(Configuration["Redis:Default:InstanceName"], p => ConnectionMultiplexer.Connect(Configuration["Redis:Default:Connection"])).GetDatabase(int.Parse(Configuration["Redis:Default:DefaultDB"].ToString()));
        //    }
        //    catch (Exception e)
        //    {
        //        LogHelper.Error(e.Message);
        ...
        ///// 获取一个key的对象
        ///// </summary>
        //public T GetObjectKey<T>(string key)
        //{
        //    if (_db == null)
        //    {
        //        return default;
