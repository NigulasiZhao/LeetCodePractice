...
{
    public class Ins_MoveTerminalDAL : IIns_MoveTerminalDAL
    {
        public MessageEntity Add(Ins_MoveTerminal model)
        {
        ...
        public MessageEntity Delete(Ins_MoveTerminal model)
        {
            using (var conn = ConnectionFactory.GetDBConn(ConnectionFactory.DBConnNames.ORCL))
            {
                var rows = 0;
        ...
        {
            string sql = @"   select * from Ins_MoveTerminal  
                             " + sqlCondition;
            var ResultList = DapperExtentions.EntityForSqlToPager<Ins_MoveTerminal>(sql, sort, ordering, num, page, out MessageEntity result, ConnectionFactory.DBConnNames.ORCL);
            return result;
        }


        public MessageEntity Update(Ins_MoveTerminal model)
        {
            using (var conn = ConnectionFactory.GetDBConn(ConnectionFactory.DBConnNames.ORCL))
            {
                var rows = 0;
