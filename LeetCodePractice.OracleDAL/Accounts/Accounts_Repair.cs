...
namespace GISWaterSupplyAndSewageServer.OracleDAL.Accounts
{
    ...
    /// 维修台账
    /// </summary>
    public class Accounts_Repair : IAccounts_Repair
    {
        public MessageEntity Add(Model.Accounts.Gis_Accounts_Repair model)
        {
        ...
        public MessageEntity Delete(Model.Accounts.Gis_Accounts_Repair model)
        {
            using (var conn = ConnectionFactory.GetDBConn(ConnectionFactory.DBConnNames.ORCL))
            {
                var rows = 0;
        ...
        {
            string sql = "select * from Gis_ACCOUNTS_REPAIR t" + sqlCondition;
            DapperExtentions.EntityForSqlToPager<Model.Accounts.Gis_Accounts_Repair>(sql, sort, ordering, num, page, out MessageEntity result, ConnectionFactory.DBConnNames.ORCL);
            return result;
        }
        ...
        public MessageEntity GetStatistics(List<ParameterInfo> parInfo, List<string> fieldsInfo, List<string> fieldsCount, string sqlCondition)
        {
            ...
            string sql = "select " + sqlFields + "," + string.Join(",", groupFields.ToArray()) + " from Gis_ACCOUNTS_REPAIR t" + sqlCondition + " group by " + sqlFields;
        ...
        public MessageEntity Update(Model.Accounts.Gis_Accounts_Repair model)
        {
            using (var conn = ConnectionFactory.GetDBConn(ConnectionFactory.DBConnNames.ORCL))
            {
                var rows = 0;
