...
namespace GISWaterSupplyAndSewageServer.OracleDAL.Accounts
{
    ...
    /// 捡漏台账
    /// </summary>
    public class Accounts_leakdetection : IAccounts_leakdetection
    {
        public MessageEntity Add(Model.Accounts.Gis_Accounts_leakdetection model)
        {
        ...
        public MessageEntity Delete(Model.Accounts.Gis_Accounts_leakdetection model)
        {
            using (var conn = ConnectionFactory.GetDBConn(ConnectionFactory.DBConnNames.ORCL))
            {
                var rows = 0;
        ...
        public MessageEntity GetList(List<ParameterInfo> parInfo, string sort, string ordering, int num, int page, string sqlCondition)
        {
            ...
            string sql = "select * from Gis_ACCOUNTS_LEAKDETECTION t" + sqlCondition; ;
            DapperExtentions.EntityForSqlToPager<Model.Accounts.Gis_Accounts_leakdetection>(sql, sort, ordering, num, page, out MessageEntity result, ConnectionFactory.DBConnNames.ORCL);
            return result;
        }
        ...
        public MessageEntity GetStatistics(List<ParameterInfo> parInfo, List<string> fieldsInfo, List<string> fieldsCount, string sqlCondition)
        {
            ...
            string sql = "select " + sqlFields + "," + string.Join(",", groupFields.ToArray()) + " from Gis_ACCOUNTS_LEAKDETECTION t" + sqlCondition + " group by " + sqlFields;
        ...
        public MessageEntity Update(Model.Accounts.Gis_Accounts_leakdetection model)
        {
            using (var conn = ConnectionFactory.GetDBConn(ConnectionFactory.DBConnNames.ORCL))
            {
                var rows = 0;
