...
namespace GISServerForCore2._0.Controllers.ApiControllers.PlanningEngine.InspectionSettings
{
   ...
   /// 事件内容管理
   /// </summary>
    public class InsEventContentController : ControllerBase
    {
        private readonly IIns_Event_TypeDAL _eventTypeDAL;
        public InsEventContentController(IIns_Event_TypeDAL eventTypeDAL)
        {
        ...
        /// <returns></returns>
        [HttpGet]
        public MessageEntity GetByEventTypeId(string eventTypeId, string sort = "EventTypeName", string ordering = "desc", int num = 20, int page = 1)
        {
            var messageEntity = _eventTypeDAL.GetEventContentInfo(eventTypeId, sort, ordering, num, page);
        ...
        {
            var messageEntity = _eventTypeDAL.GetEventContentInfo(null, sort, ordering, num, page);
        ...
        public MessageEntity Post([FromBody]Ins_Event_Type eventType)
        {
            ...
            MessageEntity messresult = _eventTypeDAL.IsExistEventContent(eventType, 1);
            List<Ins_Event_Type> ptslist = (List<Ins_Event_Type>)messresult.Data.Result;
            if (ptslist.Count > 0)
            {
                return MessageEntityTool.GetMessage(ErrorType.OprationError, "", "不允许添加重复事件内容名称！");
            }
        ...
        public MessageEntity Put(string eventTypeId, [FromBody]Ins_Event_Type eventType)
        {
            ...
            eventType.EventTypeName = eventType.EventTypeName.Replace(" ", "");
            MessageEntity messresult = _eventTypeDAL.IsExistEventContent(eventType, 0);
            List<Ins_Event_Type> ptslist = (List<Ins_Event_Type>)messresult.Data.Result;
            if (ptslist.Count > 0)
            {
                return MessageEntityTool.GetMessage(ErrorType.OprationError, "", "不允许修改为重复事件内容名称！");
            }
        ...
        public MessageEntity Delete(string eventTypeId)
        {
            ...
            //判断是否被事件内容应用
            MessageEntity messresult = _eventTypeDAL.IsExistEvent(eventTypeId);
            List<Ins_Event> ptslist = (List<Ins_Event>)messresult.Data.Result;
            if (ptslist.Count > 0)
            {
                return MessageEntityTool.GetMessage(ErrorType.OprationError, "", "该事件内容已绑定事件工单，请先删除事件工单！");
            }
