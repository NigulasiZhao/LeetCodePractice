...
namespace GISWaterSupplyAndSewageServer.Controllers.ApiControllers.PlanningEngine.Form
{
    ...
    ///泵房填单
    /// </summary>
    public class InsFormPumpController : BaseController
    {
        private readonly IIns_FormDAL _iIns_FormDAL;
        private readonly IIns_TaskManageDAL _iIns_TaskManageDAL;
        public InsFormPumpController(IIns_FormDAL iIns_FormDAL, IIns_TaskManageDAL iIns_TaskManageDAL
        ...
        [HttpPost]
        public MessageEntity Post(string ID, string x, string y, [FromBody] Ins_Form_PumpModel value, string? imageUrl = null)
        {
            if (value == null)
            {
            ...
            MessageEntity result = _iIns_FormDAL.PostPump(value, taskdetailmode, ImagePath, x, y, iadminid, iadminame, upDeptId, upDeptName);
            //调用判断任务对应的所有设备是否到位
