...
namespace GISWaterSupplyAndSewageServer.Controllers.ApiControllers.Accounts
{
    ...
    /// 维修台账
    /// </summary>
    public class Accounts_RinseController : Controller
    {
        private readonly IAccounts_Rinse _rinseDAL;

        /// <summary>
        /// 冲洗台账
        /// </summary>
        /// <param name="rinseDAL"></param>
        public Accounts_RinseController(IAccounts_Rinse rinseDAL)
        {
            _rinseDAL = rinseDAL;
        }
        ...
        /// 获取冲洗台账
        /// </summary>
        /// <param name="ParInfo">冲洗台账查询列表[{ParName:"字段名",ParValue:"字段值",DataType:"数据类型：string|number|bool|condition",LinkType:and|or}]</param>
        ...
        public MessageEntity Get(string ParInfo, string sort = "ID", string ordering = "desc", int num = 20, int page = 1)
        {
            ...
            var result = _rinseDAL.GetList(list, sort, ordering, num, page, sqlCondition);
        ...
        public MessageEntity GetStatistics(string parInfo, string fieldsInfo, string fieldsCount)
        {
            ...
            SqlCondition sqlWhere = new SqlCondition();
            string sqlCondition = sqlWhere.getParInfo(list);
            var result = _rinseDAL.GetStatistics(list, list1, list2, sqlCondition);
        ...
        /// <returns></returns>
        [HttpPost]
        public MessageEntity Post([FromBody]Gis_Accounts_Rinse value)
        {
            if (value == null)
            {
                return MessageEntityTool.GetMessage(ErrorType.FieldError);
            }
            return _rinseDAL.Add(value);
        }
        ...
        /// <returns></returns>
        [HttpPut]
        public MessageEntity Put(string ID, [FromBody]Gis_Accounts_Rinse value)
        {
            if (value == null)
            {
                return MessageEntityTool.GetMessage(ErrorType.FieldError);
        ...
        {
            return _rinseDAL.Delete(new Gis_Accounts_Rinse { ID = ID });
        }
